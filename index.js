...
const contributionsSum = GAMES_JSON.reduce((total, game) => total + game.backers, 0)
...
const raisedCardSum = GAMES_JSON.reduce((total, game) => total + game.pledged, 0)

// set inner HTML using template literal
raisedCard.innerHTML = `$${raisedCardSum.toLocaleString()}`


// grab number of games card and set its inner HTML
const gamesCard = document.getElementById("num-games");
const gamesSum = GAMES_JSON.reduce((total, game) => total + 1 , 0)

gamesCard.innerHTML = `${gamesSum.toLocaleString()}`
...
const displayStr = `
    A total of $${raisedCardSum.toLocaleString()} has been rasied for ${gamesSum}. 
    Currently, ${unfunded.length} ${unfunded !== 1 ? 'game' : 'games'} remains unfunded.
